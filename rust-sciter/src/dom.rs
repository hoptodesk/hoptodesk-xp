...
impl std::convert::TryFrom<Element> for Value {
	...
	fn try_from(e: Element) -> Result<Value> {
		let mut v = Value::new();
		let ok = if crate::api_version() >= DOM_UNWRAP_API_VERSION && _API._slot_SciterElementWrap != 0 {
			let f: extern "system" fn(*mut crate::capi::scvalue::VALUE, HELEMENT) -> SCDOM_RESULT = unsafe { std::mem::transmute(_API._slot_SciterElementWrap) };
			f(v.as_mut_ptr(), e.as_ptr())
		} else {
...
impl crate::value::FromValue for Element {
	fn from_value(v: &Value) -> Option<Element> {
		if crate::api_version() >= DOM_UNWRAP_API_VERSION && _API._slot_SciterElementUnwrap != 0 {
			let mut h = std::ptr::null_mut();
			let f: extern "system" fn(*const crate::capi::scvalue::VALUE, *mut HELEMENT) -> SCDOM_RESULT = unsafe { std::mem::transmute(_API._slot_SciterElementUnwrap) };
			let ok = f(v.as_cptr(), &mut h);
			if ok == SCDOM_RESULT::OK {
				Some(Element::from(h))
			} else {
				None
