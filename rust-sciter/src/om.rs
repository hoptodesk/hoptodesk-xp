...
/// NOTE: Requires Sciter 4.4.2.14+. Will panic on older versions.
pub fn atom(name: &str) -> som_atom_t {
	let s = s2u!(name);
	type F = extern "system" fn(LPCSTR) -> som_atom_t;
	let fp = crate::_API._slot_SciterAtomValue;
	assert!(fp != 0, "SciterAtomValue not available in this Sciter version");
	let f: F = unsafe { std::mem::transmute(fp) };
	f(s.as_ptr())
}
...
pub fn atom_name(id: som_atom_t) -> Option<String> {
	...
	let fp = crate::_API._slot_SciterAtomNameCB;
	assert!(fp != 0, "SciterAtomNameCB not available in this Sciter version");
	type F = extern "system" fn(som_atom_t, crate::capi::scdef::LPCSTR_RECEIVER, LPVOID) -> i32;
	let f: F = unsafe { std::mem::transmute(fp) };
	let ok = f(id, crate::utf::store_astr, &mut s as *mut _ as LPVOID);
	if ok != 0 {
		Some(s)
	} else {
		None
...
/// NOTE: Requires Sciter 4.4.2.16+. Will panic on older versions.
pub fn set_global<T>(asset: IAssetRef<T>) {
	let ptr = asset.as_ptr();
	type F = extern "system" fn(*mut som_asset_t) -> i32;
	let fp = crate::_API._slot_SciterSetGlobalAsset;
	assert!(fp != 0, "SciterSetGlobalAsset not available in this Sciter version");
	let f: F = unsafe { std::mem::transmute(fp) };
	f(ptr);
}
