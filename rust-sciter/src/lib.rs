...
pub fn set_variable(path: &str, value: Value) -> std::result::Result<(), ()> {
	...
	if fp == 0 { return Err(()); }
	type F = extern "system" fn(capi::sctypes::HWINDOW, capi::sctypes::LPCWSTR, *const capi::scvalue::VALUE) -> capi::sctypes::UINT;
	let f: F = unsafe { std::mem::transmute(fp) };
	let ws = s2w!(path);
	let ok = f(std::ptr::null_mut(), ws.as_ptr(), value.as_cptr());
	if ok != 0 { Ok(()) } else { Err(()) }
}
...
pub fn get_variable(path: &str) -> std::result::Result<Value, ()> {
	...
	if fp == 0 { return Err(()); }
	type F = extern "system" fn(capi::sctypes::HWINDOW, capi::sctypes::LPCWSTR, *mut capi::scvalue::VALUE) -> capi::sctypes::UINT;
	let f: F = unsafe { std::mem::transmute(fp) };
	let ws = s2w!(path);
	let mut value = Value::new();
	let ok = f(std::ptr::null_mut(), ws.as_ptr(), value.as_mut_ptr());
