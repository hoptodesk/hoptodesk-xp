...
impl Window {
  ...
  pub fn archive_handler(&mut self, resource: &[u8]) -> Result<(), ()> {
	...
	pub fn set_variable(&self, path: &str, value: crate::Value) -> std::result::Result<(), ()> {
		let fp = _API._slot_SciterSetVariable;
		if fp == 0 { return Err(()); }
		type F = extern "system" fn(crate::types::HWINDOW, crate::capi::sctypes::LPCWSTR, *const crate::capi::scvalue::VALUE) -> crate::capi::sctypes::UINT;
		let f: F = unsafe { std::mem::transmute(fp) };
		let ws = s2w!(path);
		let ok = f(self.get_hwnd(), ws.as_ptr(), value.as_cptr());
		if ok != 0 { Ok(()) } else { Err(()) }
	}
	...
	pub fn get_variable(&self, path: &str) -> std::result::Result<crate::Value, ()> {
		let fp = _API._slot_SciterGetVariable;
		if fp == 0 { return Err(()); }
		type F = extern "system" fn(crate::types::HWINDOW, crate::capi::sctypes::LPCWSTR, *mut crate::capi::scvalue::VALUE) -> crate::capi::sctypes::UINT;
		let f: F = unsafe { std::mem::transmute(fp) };
		let ws = s2w!(path);
		let mut value = crate::Value::new();
		let ok = f(self.get_hwnd(), ws.as_ptr(), value.as_mut_ptr());
